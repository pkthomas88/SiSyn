...
wrtds <- wrtds %>%
  ...
  rename(site = name) %>% 
  dplyr::select(Q, ConcDay:year) # simplifying things here, but include more columns if needed
...
wrtdsindex_annual %>% 
 filter(site %in% longtermsites, variable %in% c("ConcDay", "Q", "FluxDay")) %>% 
  ...
  geom_smooth(color = 'black', method ='lm', se= T, size = 1.5)+
  facet_wrap(~variable)+
  ggtitle('this is a bad way to plot it, + slope due to diff LTERS')
...
wrtdsindex_annual %>% 
 filter(site %in% longtermsites, variable %in% c("ConcDay", "Q", "FluxDay")) %>% 
  ...
  geom_smooth(aes(color=LTER), method ='lm', se= FALSE)+
  facet_grid(LTER~variable, scales = 'free')+
  ggtitle('maybe some interesting increases/decreases for some spots here?')
...
seas_slopes <-  wrtdsindex_annual %>%
  ...
  group_by(site, LTER, variable) %>%
  do(model = tidy(lm(wrtds_seas_index ~ year, data = .), conf.int=TRUE)) %>% # only works if you include the broom::tidy in here
  unnest(model)
...
seas_slopes%>%
  filter(term=="year", variable %in% c("ConcDay", "Q", "FluxDay"))%>%
  ggplot(aes(fill = LTER, x = paste(LTER, site, sep = "::"), y = estimate, ymin = conf.low, ymax=conf.high))+
...
seas_slopes%>%arrange(estimate)

seas_ma <- rma(estimate, std.error, data = filter(seas_slopes, term =="year"), mods = ~LTER)

seas_ma <- rma(estimate, std.error, data = filter(seas_slopes, term =="year"))
...
mafits_seas <-  seas_slopes %>%
  filter(term=="year") %>% 
  group_by(LTER, variable) %>% #group by type of nutrient manipulation and broad response
  do(model = tidy(rma(estimate, std.error, data = .), conf.int=TRUE)) %>% #do weighted meta-analysis for each grouping
  unnest(model)
  #mutate(ci.lb=estimate-(1.96*std.error), ci.ub = estimate+(1.96*std.error)) %>% #calculate 95% CIs

num_obs <- seas_slopes %>% 
filter(term=="year")%>%
  group_by(LTER, variable) %>%
  summarize(num_obs=length(estimate), mean_estimate=mean(estimate))
mafits_seas
num_obs

mafits_seas <- left_join(mafits_seas, num_obs)

mafits_seas%>%
  filter(variable %in% c("ConcDay"))%>%
  ggplot(aes(x = LTER, y = estimate, ymin = conf.low, ymax=conf.high))+
  geom_pointrange(color="black", shape = 21, size = 1.8, fill = "#218D3A")+
  scale_x_discrete(limits = rev)+
  coord_flip()+
  geom_hline(yintercept = 0, linetype="dashed")+
  geom_text(size=4, fontface="italic", position = position_nudge(x = 0.25, y=.15),
            aes(label = num_obs))+
  theme(axis.text=element_text(size=16))+
  scale_color_manual(values = jever2color)+
  scale_fill_manual(values = jever2color)+
  ...
  #scale_shape_manual(values = c(21, 22))+
  xlab("LTER")+
  ylab("Slope of annual seasonality change for DSi")
#ggsave("seasonality_annual_weighted.png")
...
mafits_seas%>%
  ...
  xlab("LTER")+
  ylab("Slope of annual seasonality change")
ggsave("seasonality_annual_weighted_conq-Q-flx.png")
...
wrtdsmonthly <- wrtds_long %>%
  #filter(variable=='ConcDay') %>% 
  group_by(LTER, site, month, year, variable) %>% 
  summarize(monthly_avg = mean(value))
...
mktests <- wrtdsmonthly %>%
  #filter(site %in% longtermsites) %>% 
  group_by(LTER, site, month, variable) %>% 
  summarize(trend=cor(year, monthly_avg, method = "kendall"), ni=n())


mktests2 <- wrtdsmonthly %>%
  group_by(LTER, site, month, variable) %>%
  do(model = tidy(lm(monthly_avg ~ year, data = .), conf.int=TRUE)) %>%
  unnest(model)

mktests%>%
  filter(variable=="ConcDay")%>%
  ggplot(aes(as.factor(month), trend))+
  #geom_point()+
  facet_wrap(~LTER)+
  geom_hline(yintercept = 0)+
  stat_summary(fun = mean, geom = "point") + 
...
mktests2%>%
  filter(term=="year", variable=="ConcDay")%>%
  ggplot(aes(as.factor(month), estimate))+
  #geom_point()+
...
mktests3 <- wrtdsmonthly %>%
  #filter(site %in% longtermsites) %>% 
  group_by(LTER, site, month, variable) %>% 
  do(model = tidy(mk.test(monthly_avg ~ year, data = .), conf.int=TRUE)) %>% 
  unnest(model)
